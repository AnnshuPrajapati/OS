...
function Implement() {
    ...
    if (inputHeadPos == "") {
        showError(ErrorMsg, "Please enter current Head position");
        isValidInput = false;
    } else if (isNaN(inputHeadPos)) {
        showError(ErrorMsg, "Only Numeric value allowed for current Head position !!!");
        isValidInput = false; 
    } else if (parseInt(inputHeadPos) < 0 || parseInt(inputHeadPos) > 199) {
        showError(ErrorMsg, "Current Head position value must be in between 1-199");
        isValidInput = false;
    } else if (inputNumbersString == "") {
        showError(ErrorMsg, "Numeric values required for Queue");
        isValidInput = false; 
    }
    else {
        inputNumbers.forEach(number => {
            if (isNaN(number)) {
                showError(ErrorMsg, "Number queue must only contain numbers");
                isValidInput = false;
            } else if (parseInt(number) < 0 || parseInt(number) > 199) {
                showError(ErrorMsg, "Number queue values must be in the range of 1-199");
                isValidInput = false;
            }
        });
    }
